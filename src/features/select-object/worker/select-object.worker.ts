...
import { bestIouMatte, normalizedPromptToPixels } from "../model/prompt-coordinates";
import {
  GUIDED_MODEL,
...
async function load(): Promise<{ model: Model; processor: Processor }> {
  if (model && processor) return { model, processor };
  post({ type: "status", status: "loading-model", progress: 0 });
  const options = {
    ...
    progress_callback: (info: { status: string; progress?: number }) => {
      if (info.status === "progress_total")
        post({ type: "status", status: "loading-model", progress: info.progress ?? 0 });
    },
  };
  try {
...
async function encode(source: SourceImage): Promise<void> {
  disposeEncoding();
  const loaded = await load();
  post({ type: "status", status: "encoding-image" });
  ...
  post({ type: "status", status: "ready-for-prompt" });
}
...
function promptInputs(
  ...
  const pixels = normalizedPromptToPixels(
    prompt,
    current.source.width,
    current.source.height,
  );
  if (pixels.type === "point") {
    ...
    return {
      input_points: rawInputPoints as Tensor,
      // The pinned split decoder graph declares both prompt inputs even for a
      // point-only request; an empty box batch satisfies that fixed ONNX input.
      input_boxes: new Tensor("float32", new Float32Array(0), [1, 0, 4]),
    };
  }
  const rawInputBoxes: unknown = loaded.reshape_input_points(
    [[[pixels.xMin, pixels.yMin, pixels.xMax, pixels.yMax]]],
    current.originalSizes,
    current.reshapedInputSizes,
    true,
  );
  return {
    input_boxes: rawInputBoxes as Tensor,
    input_points: new Tensor("float32", new Float32Array([0, 0]), [1, 1, 1, 2]),
    input_labels: new Tensor("int64", new BigInt64Array([-10n]), [1, 1, 1]),
  };
}

async function predict(prompt: SelectionPrompt): Promise<void> {
  if (!encoding || !model || !processor)
    throw new Error("Encode an image before prompting");
  post({ type: "status", status: "predicting-mask" });
...
scope.addEventListener("message", (event) => {
  ...
  if (request.type === "reset") {
    void disposeAll().then(() => post({ type: "status", status: "idle" }));
    return;
  }
  void (
    request.type === "encode" ? encode(request.source) : predict(request.prompt)
  ).catch((error: unknown) => {
    post({
      type: "error",
