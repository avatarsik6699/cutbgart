    ...
    >
      <div className="min-w-0 overflow-hidden [grid-area:toolbar]">
        <EditorToolbar
          ...
          undoLabel={projection.canUndoDraft ? m.editorDraftUndo() : null}
          redoLabel={projection.canRedoDraft ? m.editorDraftRedo() : null}
          onUndo={() =>
            props.onIntent({
      ...
      <Typography variant="caption" as="p" className="sr-only">
        {m.editorRevision({ revision: String(projection.revision) })}
      </Typography>
      {props.children}
    </div>
  );
