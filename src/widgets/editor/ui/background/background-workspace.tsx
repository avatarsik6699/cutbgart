...
import type { BackgroundTypes } from "@/editor/domain";
import type { BackgroundRuntimeSnapshot } from "@/editor/runtime";
...
function backgroundStatus(props: Props): string {
  if (props.runtime.status === "preparing-image")
    return m.editorBackgroundPreparingImage();
  if (props.runtime.status === "error") return m.editorBackgroundImageError();
  if (props.draft.status === "applying") return m.editorBackgroundApplying();
  return props.draft.dirty ? m.editorBackgroundDirty() : m.editorBackgroundClean();
}
          ...
          <section className="flex h-full min-h-0 min-w-0 max-w-full flex-col gap-5 overflow-x-hidden overflow-y-auto overscroll-contain pr-1">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  <input
                    type="color"
                    value={colour}
                    aria-label={m.editorBackgroundColorPicker()}
                    className="sr-only"
                    onChange={(event) =>
            ...
            <div>
              <label className="flex min-w-0 max-w-full cursor-pointer flex-col gap-2 text-sm font-medium">
                ...
                <input
                  ...
                  aria-label={m.editorBackgroundChooseImage()}
            ...
            {props.draft.dirty ? (
              <Typography variant="caption" as="p" className="text-muted-foreground">
                {m.editorBackgroundExportNotice()}
              </Typography>
            ) : null}
