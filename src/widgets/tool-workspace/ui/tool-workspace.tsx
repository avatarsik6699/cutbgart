...
import { BeforeAfterSlider, type QualityMode } from "@/entities/processed-image";
...
import { QualityModePopover, QualityModeToggle } from "@/features/quality-mode-toggle";
...
import type { EditorStageFullscreenControls } from "./editor-stage";
...
export function ToolWorkspace(props: Props = {}) {
  const emptyIntroSlot = props.emptyIntroSlot;
  const headerUtilityTarget = useHeaderUtilityPortalTarget();
  const controller = useToolWorkspaceController();
  const uploadError = controller.uploadError;
  const preparingFileCount = controller.preparingFileCount;
  const setPreparingFileCount = controller.setPreparingFileCount;
  const originalMatte = controller.originalMatte;
  const extractingMatte = controller.extractingMatte;
  const finalizingCorrection = controller.finalizingCorrection;
  const correctionError = controller.correctionError;
  const canvasDecodeRetryToken = controller.canvasDecodeRetryToken;
  const handleCanvasDecodeError = controller.handleCanvasDecodeError;
  const correctionViewAnnouncement = controller.correctionViewAnnouncement;
  const setCorrectionViewAnnouncement = controller.setCorrectionViewAnnouncement;
  const previewFill = controller.previewFill;
  const setPreviewFill = controller.setPreviewFill;
  const batchPreviewFills = controller.batchPreviewFills;
  const setBatchPreviewFills = controller.setBatchPreviewFills;
  const hydrated = controller.hydrated;
  const guidedVisualContext = controller.guidedVisualContext;
  const guided = controller.guided;
  const guidedViewSession = controller.guidedViewSession;
  const enhancementState = controller.enhancementState;
  const enhancementProgress = controller.enhancementProgress;
  const qualityMode = controller.qualityMode;
  const setQualityMode = controller.setQualityMode;
  const state = controller.state;
  const deviceCapabilities = controller.deviceCapabilities;
  const lightweightMode = controller.lightweightMode;
  const runInfo = controller.runInfo;
  const logs = controller.logs;
  const modelLoadBytes = controller.modelLoadBytes;
  const ben2FallbackNotice = controller.ben2FallbackNotice;
  const retry = controller.retry;
  const retryInLightweightMode = controller.retryInLightweightMode;
  const applyBackgroundFill = controller.applyBackgroundFill;
  const releaseRefinementBeforeHeavyWork = controller.releaseRefinementBeforeHeavyWork;
  const batch = controller.batch;
  const batchModelKey = controller.batchModelKey;
  const selectedBatchItem = controller.selectedBatchItem;
  const activeEditDocument = controller.activeEditDocument;
  const historySelectors = controller.historySelectors;
  const handleUpload = controller.handleUpload;
  const handleUploads = controller.handleUploads;
  const handleDismissUploadError = controller.handleDismissUploadError;
  const handleReset = controller.handleReset;
  const handleApplyGuided = controller.handleApplyGuided;
  const handleGuideAutomaticResult = controller.handleGuideAutomaticResult;
  const handleGuideBatchResult = controller.handleGuideBatchResult;
  const synchronizeSingleGuidedTarget = controller.synchronizeSingleGuidedTarget;
  const synchronizeBatchGuidedTarget = controller.synchronizeBatchGuidedTarget;
  const applySingleEnhancements = controller.applySingleEnhancements;
  const applyBatchEnhancements = controller.applyBatchEnhancements;
  const cancelEnhancements = controller.cancelEnhancements;
  const retryEnhancements = controller.retryEnhancements;
  const handleRetry = controller.handleRetry;
  const handleEditMask = controller.handleEditMask;
  const handleBatchEditMask = controller.handleBatchEditMask;
  const handleSelectBatchItem = controller.handleSelectBatchItem;
  const handleClearBatch = controller.handleClearBatch;
  const handleBatchDoneCorrecting = controller.handleBatchDoneCorrecting;
  const handleDoneCorrecting = controller.handleDoneCorrecting;
  const handleCancelCorrection = controller.handleCancelCorrection;
  const handleUndoDocument = controller.handleUndoDocument;
  const handleRedoDocument = controller.handleRedoDocument;
  const commitSingleBackground = controller.commitSingleBackground;
  const commitBatchBackground = controller.commitBatchBackground;
  const cancelGuided = controller.cancelGuided;
  ...
  useEffect(
    function initializeMagicDocumentFx() {
      if (
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- active document identity intentionally owns this transition.
    [
      activeDocumentVersion,
  ...
  useEffect(
    function initializeManualDocumentFx() {
      if (
        !activeDocumentVersion ||
        documentUiState.activeTool !== "cutout" ||
      ...
      else if (state.status === "result" || state.status === "correcting")
        handleEditMask();
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps -- active document identity intentionally owns this transition.
    [
  ...
  useEffect(
    function selectFirstReadyBatchItemFx() {
  ...
  if (!displayError && state.status === "idle" && !batch.session.items.length) {
    surfaceNode = guidedCanvas ?? (
      <MainPageEmptySurface
        ...
        preparationSlot={<UploadPreparationNotice fileCount={preparingFileCount} />}
        errorSlot={
          uploadError ? (
