...
async function counters(page: Page): Promise<Phase42Counters> {
  return page.evaluate(() => {
    const testWindow = window as Window & { __phase42Counters?: Phase42Counters };
    return (
      testWindow.__phase42Counters ?? {
        automatic: 0,
        backgroundCommits: 0,
        enhancementRuns: 0,
        magicCommits: 0,
...
test("serialized real model covers cold and warm complete-product work without reinference", async ({
  ...
  await page.addInitScript(() => {
    ...
    function recordWorkerMessage(counters: Phase42Counters, value: unknown): void {
    ...
    Worker.prototype.postMessage = function (
      ...
      const current = testWindow.__phase42Counters!;
