# PHASE 25 — Editor Document Foundation & Guided Reset
...
for both a single image and every completed batch item, required by the Phase-26–30 editor redesign
...
- Phase-26 toolbar/layout, automatic-mode naming, or removal of public technical copy
- Candidate ranking/fusion algorithms, model IDs/revisions/assets, matting/foreground algorithms
- Batch visual redesign or batch-wide editing; Phase 25 only establishes per-item document ownership
...
Run `/phase-gate 25` after all Scope items and review notes are resolved. In addition to the
...
Phase 25 cannot close if the final-stroke flow remains disabled, a zero-mark base reset posts to
...
The Phase-24 serialized real-model Chromium timeout is an inherited gate item: rerun or investigate
it during this phase and do not close Phase 25 while the result remains neither passing nor
explicitly resolved by a new architect decision.
...
```text
refactor(phase-25): add editor document history and guided reset
```
