...
- Phase 21 brush strokes, compact constraint maps, prompt samples, candidates, edit regions,
  ...
  discarded on reset/source change/unmount. Only
  `docs/archive/research/PHASE_21_RUNTIME_EVIDENCE.md` persists, with
...
**Triggered by**: concern that accumulated phase contracts, audits, evaluations, drafts, and build
artifacts could obscure the current v2 sources of truth.

### Changes / Decision
- `docs/README.md` is now the entry point and names the small active set: SPEC, STATE,
  ARCHITECTURE_V2, PHASE_33, STACK, FRONTEND_CONVENTIONS, KNOWN_GOTCHAS, design system, playbooks,
  security/operations/runbooks, and active-phase evidence.
- Completed/closed Phase 01–32 and stale Phase 34 moved to `docs/archive/phases/`; superseded plans,
  Phase-31/32 audits, model/matting research/runtime evidence, Phase-24 legal baseline/drafts, and
  Phase-30 screenshots moved to typed archive directories indexed by `docs/archive/README.md`.
- Archived material is traceability input only and cannot define current scope. Phase-24 legal
  material remains recoverable for the mandatory future refresh before paid/server functionality.
- Ignored reproducible `.output/`, `.tanstack/`, and `test-results/` directories were removed.
  `node_modules` remains as the working dependency cache.
- No product/runtime contract or source behavior changed.

### Affected Phases / Consequences
- PHASE_33 — remains the only active implementation phase; its future evidence uses `docs/audits/`.
- PHASE_34 — stale contract retained under the archive and remains `NEEDS_REVIEW` in the tracker.

## 2026-08-01 — V2 frontend foundation and architect review made explicit

**Type**: spec-change
**Author**: AI (spec-sync), architect decision
**Triggered by**: architect request to rewrite v2 presentation components and establish shared
Typography/Image/config/runtime/utilities from the start under `FRONTEND_CONVENTIONS.md`.

### Changes / Decision
- Phase 33 now builds a v2-owned shared presentation foundation instead of inheriting the legacy
  component tree: typed semantic Typography, optimized/preset Image, and only consumer-proven
  generic components/utilities.
- Repository-wide `shared/config/env.ts` and new `runtime.ts` adopt the SSR-safe pattern from
  `patient_tracker`, with typed client/server separation, validation, dynamic runtime detection,
  no client secret exposure, and backward-compatible legacy exports during migration.
- Existing compliant storage/router helpers may be reused through public APIs after inventory;
  direct platform/env/storage/worker/object-URL access outside the owning wrapper is forbidden.
- Every new public API receives same-commit unit/component coverage and file-level convention
  review. Architect review of code structure, SSR behavior, shared APIs, and product behavior is a
  blocking Phase-33 acceptance step.
- No v2 runtime code or dependency is implemented by this spec edit, so Current Contract remains
  unchanged.

### Affected Phases / Consequences
- PHASE_33 — scope, files, contracts, dependencies, tests, and post-phase review now include the
  v2 frontend/shared foundation.
- PHASE_34 — unaffected; it remains `NEEDS_REVIEW` for the previously recorded roadmap reason.
...
- `docs/archive/phases/PHASE_31.md` gains `T7` (test-suite reliability/speed inventory), `T8` (empty/error/warning/
...
### Changes / Decision
- `docs/archive/phases/PHASE_31.md` and `docs/archive/phases/PHASE_32.md` swap content: PHASE_31 is now "Whole-Project Audit &
...
- The uncommitted Pencil→Claude Design tooling swap (a prior session's in-progress edit to
  `docs/archive/phases/PHASE_30.md`/`SPEC.md`/`STACK.md`/`STATE.md` that was never committed) is discarded outright
...
- `SPEC.md` v1.21 (§1.3 target/scope table, §5 design-reference blockquote, §5.2/§5.3/§5.4 phase
  ...
  `docs/archive/legal-phase-24/*.md` forward references to PHASE_31–35 were renumbered to match.

### Affected Phases / Consequences
- PHASE_30 — redefined in place (`docs/archive/phases/PHASE_30.md` rewritten, not file-renamed); now permits `src/`
...
### Affected Phases / Consequences
- PHASE_30, PHASE_31 — new pending phase docs created (`docs/archive/phases/PHASE_30.md`, `docs/archive/phases/PHASE_31.md`);
  PHASE_31 folds in every Scope task from the removed `docs/archive/phases/PHASE_30.md` (batch consolidation).
...
### Affected Phases / Consequences
- PHASE_25 may implement only
  `docs/archive/legal-phase-24/IMPLEMENTATION_MATRIX.md`: analytics is fail-closed before
