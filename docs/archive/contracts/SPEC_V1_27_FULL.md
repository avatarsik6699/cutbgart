...
| `v2/shared/ui` | v2 shared UI | (Phase 33) Rewritten Typography and optimized Image primitives plus only the generic presentation components consumed by proven v2 slices. Semantic/visual variants, image loading/decoding/fetch-priority/accessibility policy, and public APIs obey `FRONTEND_CONVENTIONS.md`. |
| `v2/shared/lib` | v2 shared utilities | (Phase 33+) Consumer-driven, tested browser/image/abort/error helpers. Reuse compliant repository-wide storage/router utilities through public APIs; no speculative helper catalog or direct platform access from components. |
| `shared/config` | repository-wide SSR boundary | Typed `env.ts` + `runtime.ts`, adapted from `patient_tracker`: the sole `import.meta.env` readers, separate client/server config, validate consumed values, expose dynamic SSR/client/window state, and preserve compatible legacy exports during migration. |
...
| `33` | Editor v2 Foundation & First Vertical Slice | Establish the new domain/runtime/presentation architecture and prove one responsive end-to-end local workflow before migrating editor breadth | `docs/ARCHITECTURE_V2.md`; framework-free domain/application modules; XState document/workspace actors; artifact repository; unified worker protocol and bounded local processing gateway; rewritten v2 Typography/Image/shared utilities; typed SSR-safe `env.ts`/`runtime.ts`; separately reachable single-image import → local automatic removal → preview → PNG export; file-level `FRONTEND_CONVENTIONS.md` compliance; deterministic contract/component/model tests, bilingual E2E, real-model smoke, artifact audit, architect-device trace and blocking architect code/behavior review; legacy route retained; no Cutout, Enhancements, Background, batch, accounts, payments, or remote API in this phase |
