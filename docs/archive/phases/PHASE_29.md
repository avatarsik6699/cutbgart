# PHASE 29 — Background & Export Tools
...
- [x] `F1` Move transparent/solid/gradient/uploaded-image choices into the Background tool panel
...
- [x] `F2` Preview background draft changes live behind the cutout, mark the panel dirty, and keep
...
- [x] `F3` Integrate Background operations with toolbar undo/redo, including uploaded-image blob
  reachability and cleanup after undo-branch replacement, reset, or history eviction — _Depends
  on:_ `F2`
- [x] `F4` Replace the rail download button with a toolbar split action: primary Download uses the
...
- [x] `F5` Add `ExportSettings` with PNG and longest-side choices `original`, `2048`, `1024`.
...
- [x] `F6` Generate resized output off the main interaction path with cancellation/latest-request
...
- [x] `F7` Show no disabled WebP/JPEG controls. Keep `format: "png"` in the typed contract so later
  approved formats can extend the exporter without changing toolbar ownership — _Depends on:_ `F5`
- [x] `F8` Add accessible split-button/menu keyboard behavior, selected-size announcement,
...
- [x] `F9` Add unit/component/integration tests for Background draft/apply/cancel/history, uploaded
  ...
  _Depends on:_ `F1`–`F8`
- [x] `F10` Add bilingual cross-browser Playwright coverage: change each background type, Cancel
...
- [x] `F11` Keep Download all as a client-generated ZIP of each item's committed result. Preserve
...
- [x] `I1` Use existing browser canvas/worker capabilities; add no export library, server endpoint,
...
Run `/phase-gate 29`; standard checks plus:
...
```text
feat(phase-29): add background drafts and sized PNG export
```

## Post-Phase Checklist

- [x] Scope complete; gates green; review notes resolved
- [x] Run `/context-update 29`
- [x] Commit on `feat/phase-29`; tag `v0.29.0` after merge
