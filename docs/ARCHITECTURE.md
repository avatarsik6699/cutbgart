# BG Remove App — Editor Architecture

**Status:** implemented current architecture; permanent paths normalized in Phase 44

**Date:** 2026-08-07
**Purpose:** define the current editor domain, application, browser-runtime, presentation, testing,
and dependency boundaries. Migration sections are retained as decision history; paths and normative
rules describe the sole current implementation.

## 1. Executive decision

Phases 33–43 replaced the former editor through isolated vertical slices. The first slice was:
...
The editor did not inherit the legacy component tree by default. It built a small, reviewed UI foundation
...
| UI composition | `src/widgets/editor` | Stable model/provider boundary, leaf subscriptions, and focused editor presentation |
| Workflow state | `src/editor/application` | Workspace/document actors, commands, selectors, and application ports |
| Domain data | `src/editor/domain` | IDs, documents, artifacts, commands, events, and transition policies |
| Browser adapters | `src/editor/runtime` | Sessions, artifact ownership, workers, controllers, export, and platform adapters |
...
```text
File
  -> EditorModel / EditorSession command
  -> import preparation + ArtifactRepository
  -> workspace/document actors
  -> typed processing/controller port
  -> dedicated worker
  -> correlated result and document commit
  -> narrow model/XState selector at the consuming React connector
  -> controller-neutral view / leased preview / PNG export
```

Batch mode is coordinated by the workspace/session owners over per-document actors. React does not
mirror queue, run, revision, selection, or artifact truth; leaf connectors subscribe only to the
fields they render and invoke semantic model/session commands.

### 2.3 Current domain vocabulary

| Concept | Meaning | Owner |
|---------|---------|-------|
| `ImageDocument` / document snapshot | Small committed document state and revision | document actor in `src/editor/application` |
| `ArtifactId` and leases | Opaque references to large browser resources | `ArtifactRepository` in `src/editor/runtime` |
| workspace/document commands | Semantic user and runtime intent | application actors |
| processing runs | Correlated progress, cancellation, and terminal outcomes | application port plus runtime gateway/client |
| editor view model | Stable session access, cached snapshots, semantic commands | `src/widgets/editor/model` |
| local interaction state | Ephemeral visual state needed by one control/tool | nearest component or capability hook |

### 2.4 Historical problem evidence
...
This evidence explains the replacement decision. The current editor addresses it with document and
workspace actors, a stable session/model boundary, typed worker protocols, and runtime-owned
resources; Phase 44 continues decomposing the React render surface without changing those owners.
...
The processing port has remained stable since the first replacement slice:
...
### 5.5 Editor presentation and shared foundation

The editor consumes repository-wide presentation primitives from `src/shared/ui` and generic hooks
from the repository-wide `src/shared/lib` public API instead of composing bespoke
...
state never cross into the editor core.
...
typed `env.ts` + `runtime.ts` shape with backward-compatible legacy exports; the editor consumes
only the namespaced API. Secrets are never exposed through client config.
...
### 6.2 Added by the editor foundation
...
| Typography/Image primitives | consistent semantics, rendering presets, and image-loading policy |
...
```text
src/editor/domain/             # pure entities, value objects, commands, events, invariants
src/editor/application/        # actors/use cases, ports, policies, selectors
src/editor/runtime/            # worker pool, local processing adapter, artifact repository adapters
src/widgets/editor/ui/         # React bindings and editor UI composition
src/editor/testing/            # deterministic fakes, model-based test helpers
...
Editor-core modules do not import feature hooks or use React state as source of truth. FSD remains
the delivery-layer vocabulary; the domain/application/runtime core is role-oriented because forcing
those responsibilities into UI layer names would obscure ownership.

The accepted public editor is the visual and interaction contract. Architecture isolation does not
imply a new product design. Presentation connects to narrow typed selectors and semantic commands;
it does not pull browser resources, actor internals, or worker ownership through the React seam.
...
### Subsequent slices (completed through Phase 43)
...
The old editor was removed after the replacement reached feature parity and passed architect
verification in Phase 43. Phase 44 removes the remaining migration-era structure and terminology;
the `/editor-v2` URLs remain redirect-only compatibility routes.
...
Use a small test kit under `e2e/support/editor/`:
...
`src/editor/testing/`. Use typed builders with minimal valid defaults, explicit scenario overrides,
...
The editor establishes:

```text
src/editor/testing/performance/
...
type DocumentCommand = Exclude<EditorCommand, { type: "IMPORT_IMAGE" }>;

type CommandOutcome =
  | { status: "accepted"; command: EditorCommand["type"] }
  | { status: "rejected"; command: EditorCommand["type"]; reason:
...
Only `src/editor/runtime/worker/processing.worker.ts` and its gateway/client adapter may use the
...
|------|---------|----------------------|
| `src/editor/domain/ids.ts` | D | branded opaque IDs and injected generation contract only |
| `src/editor/domain/artifacts.ts` | D | artifact metadata/lease types contain no binary value |
| `src/editor/domain/document.ts` | D | snapshot/revision types contain IDs and small metadata only |
| `src/editor/domain/commands.ts` | D | pure document commands; browser `File` excluded |
| `src/editor/domain/events.ts` | D | metadata-only discriminated events and terminal outcomes |
| `src/editor/domain/processing.ts` | D | processing correlation, progress, errors, and terminal contracts |
| `src/editor/domain/capabilities.ts` | D | small browser-processing support metadata only |
| `src/editor/domain/transitions.ts` | D | pure legal transition and typed rejection function |
| `src/editor/domain/index.ts` | D | explicit domain public API only |
| `src/editor/application/workspace-machine.ts` | A | `setup()` parent and `spawnChild`/`stopChild`; one document cap |
| `src/editor/application/document-machine.ts` | A | sole document writer; injected processing actor; ID-only context |
| `src/editor/application/processing-gateway.ts` | A | backend-neutral port with progress/cancel/release/dispose |
| `src/editor/application/selectors.ts` | A | narrow primitive/ID/status selectors; no binary/actor internals |
| `src/editor/application/index.ts` | A | explicit application public API only |
| `src/editor/runtime/artifact-repository.ts` | B | deterministic leases, promotion, stats, budget, assertions |
| `src/editor/runtime/artifact-id-source.ts` | B | sole browser randomness owner for opaque artifact IDs |
| `src/editor/runtime/artifact-url-adapter.ts` | B | sole object URL creation/revocation owner |
| `src/editor/runtime/browser-capabilities.ts` | B | sole dynamic browser/GPU/OffscreenCanvas capability owner |
| `src/editor/runtime/local-processing-gateway.ts` | B | one-heavy-job queue and warm-runtime policy |
| `src/editor/runtime/processing-cancellation.ts` | B | sole actor-facing AbortController factory; application receives a typed cancellation port |
| `src/editor/runtime/editor-id-source.ts` | B | sole session/document/image/run randomness adapter |
| `src/editor/runtime/download-adapter.ts` | B | sole native download interaction; URL lifetime stays in repository |
| `src/editor/runtime/editor-session.ts` | B | workspace composition root; import, preview/export lease, and route-disposal ownership |
| `src/editor/runtime/worker-client.ts` | B | correlation, progress order, terminal/crash/transfer enforcement |
| `src/editor/runtime/worker-factory.ts` | B | sole `new Worker`/termination owner outside the worker global |
| `src/editor/runtime/worker-protocol.ts` | D | pure versioned discriminated message/transfer types |
| `src/editor/runtime/model-config.ts` | D | immutable existing model asset configuration only |
| `src/editor/runtime/worker/processing.worker.ts` | W | sole worker implementation and full-resolution compute owner |
| `src/editor/runtime/index.ts` | B | explicit runtime public API; no leaked native worker/repository internals |
...
| `src/widgets/editor/ui/use-document-actor-selectors.ts` | P | selector-only `@xstate/react` binding over narrow application selectors |
| `src/widgets/editor/ui/index.ts` | P | explicit presentation public API only |
| `src/widgets/editor/model/` | P | stable session/view-model lifetime, cached external-store binding, semantic commands, and pure public-admission policies without mirrored workflow state |
| `src/widgets/editor/ui/editor.tsx` | P | thin provider/surface root and route-lifetime resource-count synchronization |
| `src/widgets/editor/ui/connectors/` | P | leaf subscriptions that adapt session/XState owners to controller-neutral presentation contracts |
| `src/widgets/editor/ui/image-stage.tsx` | P | one-image input and leased Typography/Image preview only |
| `src/widgets/editor/ui/active-document.tsx` | P | active-document connector and accepted tool/canvas composition |
| `src/widgets/editor/ui/tool-workspace.tsx` | P | active tool workspace composition |
...
| `src/widgets/editor/ui/diagnostics/` | P | diagnostics overlay capability with declarative `react-error-boundary` plus React `lazy`/`Suspense` chunk containment |
| `src/widgets/editor/ui/editor-tools/` | P | controller-neutral toolbar, canvas controls, tool panels, registries, and focused tests grouped by editor-tool responsibility |
| `src/widgets/editor/ui/main-page/image-admission/` | P | controller-neutral empty input/quality/error and batch-capacity presentation with semantic callbacks |
| `src/widgets/editor/ui/main-page/single-image/` | P | controller-neutral preparation, progress, recovery, comparison, and result presentation with narrow child props |
...
| `src/editor/testing/builders.ts` | T | minimal valid typed values and explicit overrides |
| `src/editor/testing/fake-clock.ts` | T | deterministic application clock, no ambient fake timer leak |
| `src/editor/testing/fake-ids.ts` | T | deterministic ID source, reset per test |
| `src/editor/testing/fake-processing-gateway.ts` | T | implements production port, not a second state machine |
| `src/editor/testing/worker-scenario-driver.ts` | T | drives production protocol and observable scenarios |
| `src/editor/testing/index.ts` | T | test-only public API with explicit cleanup contracts |
| `src/editor/testing/performance/contracts.ts` | M | typed signals, support flags, budgets, versioned report schema |
| `src/editor/testing/performance/marks.ts` | M | finite mark registry and injected performance adapter |
| `src/editor/testing/performance/browser-collector.ts` | M | sole observer/Event Timing/User Timing collector owner |
| `src/editor/testing/performance/resource-probe.ts` | M | repository/worker/session/listener counts only |
| `src/editor/testing/performance/report.ts` | M | percentile and unsupported-aware budget evaluator |
| `src/editor/testing/performance/test-adapter.ts` | T | deterministic samples/clock without browser globals |
| `e2e/support/editor/fixtures.ts` | E | typed context-isolated setup and automatic cleanup |
| `e2e/support/editor/scenario-driver.ts` | E | supported test port; no direct private hook patching |
| `e2e/support/editor/upload.ts` | E | narrow upload component object; actions only |
| `e2e/support/editor/progress.ts` | E | narrow progress component object; no hidden assertions |
| `e2e/support/editor/preview.ts` | E | narrow preview component object; no CSS/XPath |
| `e2e/support/editor/export.ts` | E | narrow export component object; observable download action |
| `e2e/phase-33-editor.spec.ts` | E | bilingual deterministic parallel-safe journey |
| `e2e/support/mock-editor-worker.ts` | E | deterministic production-protocol adapter; no model/CDN/GPU dependency |
| `e2e/phase-33-editor.real.spec.ts` | E | one serialized real boundary smoke; no duplicate UI coverage |
| `scripts/profiling/editor/run-phase-43.mjs` | M | final cutover report-contract orchestration shell; no product logic |
...
| browser capability | R2 owner | `editor/runtime/processing/browser-capabilities.ts` reports Worker, OffscreenCanvas, WebGPU/WASM and support flags through one tested adapter; no component probes globals |
...
policies were rewritten behind isolated editor module boundaries. No legacy React hook, editor store, worker
lifecycle, workflow state, or component crosses into the current editor core.
...
workspace state, and worker lifecycle never cross into the current editor core.
